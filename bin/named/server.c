...
ta_fromconfig(const cfg_obj_t *key, bool *initialp, const char **namestrp,
	...
	// OQS updated from 4096 to 65535
	unsigned char data[65535];
	isc_buffer_t databuf;
	// OQS updated from 4096 to 65535
	unsigned char rrdata[65535];
...
configure_peer(const cfg_obj_t *cpeer, isc_mem_t *mctx, dns_peer_t **peerp) {
	...
	if (obj != NULL) {
		...
		// OQS changed from 4096 to 65535
		if (udpsize > 65535U) {
			udpsize = 65535U;
		}
...
#define CAP_IF_NOT_ZERO(v, min, max) \
	...
	// OQS updated from 4096 to 65535
	CAP_IF_NOT_ZERO(recv_tcp_buffer_size, 65535, INT32_MAX);
	...
	// OQS updated from 4096 to 65535
	CAP_IF_NOT_ZERO(send_tcp_buffer_size, 65535, INT32_MAX);
	...
	// OQS updated from 4096 to 65535
	CAP_IF_NOT_ZERO(recv_udp_buffer_size, 65535, INT32_MAX);

	obj = NULL;
	result = named_config_get(maps, "udp-send-buffer", &obj);
	INSIST(result == ISC_R_SUCCESS);
	send_udp_buffer_size = cfg_obj_asuint32(obj);
	CAP_IF_NOT_ZERO(send_udp_buffer_size, 65535, INT32_MAX);
