...
create_validator(dns_validator_t *val, dns_name_t *name, dns_rdatatype_t type,
	...
	result = dns_validator_create(val->view, name, type, rdataset, sig,
				      val->message, vopts, val->loop, cb, val,
				      &val->subvalidator);
	if (result == ISC_R_SUCCESS) {
...
validate_answer(dns_validator_t *val, bool resume) {
	...
	if ((val->message->flags & DNS_MESSAGEFLAG_Z) != 0) {
		printf("\n\nBIND9: Z flag is set. Response already "
		       "authenticated by daemon.\n\n");
		marksecure(val);
		validator_log(val, ISC_LOG_DEBUG(3),
			      "marking as secure, "
		...
		printf("\n\nBIND9: Z flag is not set. Continue with "
		       "verification.\n\n");
	}
